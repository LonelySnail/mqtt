...
function MQTT.client:connect(                                     -- Public API
  ...
  local errMsg = " no errMsg"
  self.socket_client, errMsg = socket.connect(self.hostname, self.port)
  print("socket connect errMsg : ", errMsg)
...
function readPacket(self)
    ...
    local type_flags = string.byte(data, 1)
    ...
    repeat
        ...
        length = length + multiplier * (c >= 128 and (c - 128) or c)
  ...
  local data, err = self:receive(length)
  if not data then
    return nil, err,0
  end 

  return type_flags, data,length
end
...
function MQTT.client:parse_message(                             -- Internal API
  ...
  local message_type = MQTT.Utility.shift_right(message_type_flags, 4)
  -- local  message_type  = message_type_flags
...
function MQTT.client:parse_message_publish(                     -- Internal API
  ...
  if (self.callback ~= nil) then
    ...
    -- if (qos > 0) then 时出现数据错误（临时处理方案，直接callback错误数据 client中做重连处理）
